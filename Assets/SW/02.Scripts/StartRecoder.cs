...
public class StartRecoder : MonoBehaviour
{
    ...
    private void OnAudioFilterRead(float[] data, int channels)
    {
        if(recOutput)
        {
            ConverAndWrite(data); // audio data is interlaced
        }
    }

    private void ConverAndWrite(float[] dataSource)
    {
    ...
    private void WriteHeader()
    {
        ...
        fileStream.Write(riff, 0, riff.Length);
        ...
        fileStream.Write(fmt, 0, fmt.Length);
